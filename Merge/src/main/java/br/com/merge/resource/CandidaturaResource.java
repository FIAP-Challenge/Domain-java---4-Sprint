...
public class CandidaturaResource {
	...
	public List<Candidatura> listaCandidatos() throws ClassNotFoundException, SQLException, IdNotFoundException {
		try {
			...
			return (List<Candidatura>) Response.status(400, e.getMessage()).entity("\"mensagem\":" + "\"" + e.getMessage() + "\"").type(MediaType.APPLICATION_JSON).build();
	...
	public Response busca(
			@PathParam("vaga") int codigoVaga,
			@PathParam("candidato") int codigoCandidato)
