...
public class CandidatoDao {
	...
	public void atualizar(Candidato candidato) throws SQLException, IdNotFoundException {
		...
		PreparedStatement stmt = conexao.prepareStatement("UPDATE T_MERGE_CANDIDATO SET NM_CANDIDATO = ?, NR_CPF = ?, DT_NASCIMENTO = to_date(?,'dd/mm/yyyy'), DS_ESTADO_CIVIL = ?, DS_SEXO = ?, DS_EMAIL = ?, PS_SENHA = ? where ID_CANDIDATO = ?");
	...
	public Candidato select(String cpf) throws SQLException, IdNotFoundException, ClassNotFoundException {
		...
		Candidato candidato = new Candidato(codigo, nome, cpf, sexo, email, senha, estadoCivil, dataNascim, telefoneCandidato, enderecoCandidato, curriculoCandidato, statusLogin, tipoLogin, discCandidato);
	...
	public Candidato select(String emailLogin, String senhaLogin) throws SQLException, IdNotFoundException, ClassNotFoundException {
		PreparedStatement stmt = conexao.prepareStatement("select * from T_MERGE_CANDIDATO where DS_EMAIL = ? AND PS_SENHA = ?");
